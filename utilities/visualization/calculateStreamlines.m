...
switch visType
    case 'centerline contours'
        ...
        for ii = ptRange
            ...
            currL  = find(B(:));
            if isempty(currL)
                warning('calculateStreamlines: regiongrowing returned empty mask for centerline point %d.', ii);
                continue
            end
            ...
            % Bound them to ensure they stay within the matrix dimensions
            idx_X = min(max(idx_X, 1), size(currSeg, 2));
            idx_Y = min(max(idx_Y, 1), size(currSeg, 1));
            idx_Z = min(max(idx_Z, 1), size(currSeg, 3));

            % Convert the rounded subscripts to linear indices
            % Note: sub2ind expects (rows, cols, pages) which is (Y, X, Z) in MATLAB
            linearIndices = sub2ind(size(currSeg), idx_Y, idx_X, idx_Z);

            startX = [startX; xcoor_grid(linearIndices)];
            startY = [startY; ycoor_grid(linearIndices)];
            startZ = [startZ; zcoor_grid(linearIndices)];
        end
...
if useParallel && strcmp(visType,'segmentation')
    % DataQueue allows the worker to send progress back to the client.
    ...
    parfor t = 1:nframes
        ...
        for k = 1:nbins
            Xb{k,t} = Xb_t{k}; %#ok<PFPIE>
            Yb{k,t} = Yb_t{k}; %#ok<PFPIE>
            Zb{k,t} = Zb_t{k}; %#ok<PFPIE>
            Cb{k,t} = Cb_t{k}; %#ok<PFPIE>
        end
        send(dq, t);
    end
